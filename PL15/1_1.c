  ...
  for (int k = 0; k < N; k++){
    for (int j = 0; j < N; j+= 4){
      ...
      for (int i = 0; i < N; i += 4){
        ...
        C[i+ N*k] += A[i1+N*j]*B[j+N*k];
        C[i+ N*k] += A[i1+N*j1]*B[j1+N*k];
        C[i+ N*k] += A[i1+N*j2]*B[j2+N*k];
        C[i+ N*k] += A[i1+N*j3]*B[j3+N*k];

        C[i1+ N*k] += A[i1+N*j]*B[j+N*k];
        C[i1+ N*k] += A[i1+N*j1]*B[j1+N*k];
        C[i1+ N*k] += A[i1+N*j2]*B[j2+N*k];
        C[i1+ N*k] += A[i1+N*j3]*B[j3+N*k];

        C[i2+ N*k] += A[i1+N*j]*B[j+N*k];
        C[i2+ N*k] += A[i2+N*j1]*B[j1+N*k];
        C[i2+ N*k] += A[i2+N*j2]*B[j2+N*k];
        C[i2+ N*k] += A[i2+N*j3]*B[j3+N*k];

        C[i3+ N*k] += A[i3+N*j]*B[j+N*k];
